class TableTracker:
    ...
    def __init__(self, threshold=5):
        ...
        self.threshold = threshold   # сколько кадров нужно для подтверждения
        self.events = []

    def update(self, has_person, timestamp):
        """Преобразует сигнал в событие"""
        if has_person:
            self.buffer += 1   # накапливаем сигнал
        else:
            self.buffer -= 1

        self.buffer = max(0, min(self.buffer, self.threshold))  # ограничиваем диапазон [0, 5]
        stable_has_person = self.buffer >= self.threshold   # считаем "реально есть человек", человек должен быть 5 кадров → событие
